...
class SeparatedReplayBuffer(object):
    ...
    def __init__(self, config, obs_space, share_obs_space, act_space):
        ...
        self.aver_episode_costs = torch.zeros(self.episode_length + 1,
                                              self.n_rollout_threads,
                                              *obs_shape,
                                              device=self.device)  # 每个episode的平均成本
        ...
        self.masks = torch.ones(self.episode_length + 1,
                                self.n_rollout_threads,
                                1,
                                device=self.device)  # 存储掩码，指示episode是否结束。
        self.bad_masks = torch.ones_like(self.masks)  # 存储坏的掩码，指示不好的episode。
        self.active_masks = torch.ones_like(self.masks)  # 存储活跃的掩码，指示活跃的episode。

        self.factor = torch.ones(self.episode_length, self.n_rollout_threads, 1, device=self.device)  # 用于损失值计算
    ...
    def compute_returns(self, next_value, value_normalizer=None):
        ...
        for step in reversed(range(self.rewards.shape[0])):
            ...
            self.returns[step] = gae + value_normalizer.denormalize(
                self.value_preds[step])  # GAE提供了当前动作相对状态值函数$V(s_t)$的优势，加上这个值，就是回报值
...
class SharedReplayBuffer(object):
    def __init__(self, config, num_agents, obs_space, share_obs_space, act_space):
        self.episode_length = config["episode_length"]
        self.n_rollout_threads = config["n_rollout_threads"]
        self.rnn_hidden_size = config["hidden_size"]
        self.recurrent_N = config["recurrent_N"]
        ...
        self.use_share_obs = config["use_share_obs"]
        ...
        self.share_obs = torch.zeros(self.episode_length + 1,
                                     ...
                                     *share_obs_shape,
                                     device=self.device)
        self.obs = torch.zeros(self.episode_length + 1, self.n_rollout_threads, self.num_agents, *obs_shape,
                               device=self.device)

        self.rnn_states = torch.zeros(self.episode_length + 1, self.n_rollout_threads, self.num_agents,
        ...
        self.value_preds = torch.zeros(self.episode_length + 1, self.n_rollout_threads, self.num_agents, 1,
                                       device=self.device)
        self.returns = torch.zeros(self.episode_length + 1, self.n_rollout_threads, self.num_agents, 1,
                                   device=self.device)

        if act_space.__class__.__name__ == 'Discrete':
            self.available_actions = torch.ones(self.episode_length + 1, self.n_rollout_threads, self.num_agents,
                                                act_space.n, device=self.device)
        else:
            self.available_actions = None

        act_shape = act_space.shape[0]

        self.actions = torch.zeros(self.episode_length, self.n_rollout_threads, self.num_agents, act_shape,
                                   device=self.device)
        self.action_log_probs = torch.zeros(self.episode_length, self.n_rollout_threads, self.num_agents, act_shape,
                                            device=self.device)
        self.rewards = torch.zeros(self.episode_length, self.n_rollout_threads, self.num_agents, 1, device=self.device)

        self.costs = torch.zeros_like(self.rewards)
        self.cost_preds = torch.zeros_like(self.value_preds)
        self.cost_returns = torch.zeros_like(self.returns)

        self.masks = torch.ones(self.episode_length + 1, self.n_rollout_threads, self.num_agents, 1, device=self.device)
        self.bad_masks = torch.ones_like(self.masks)
        self.active_masks = torch.ones_like(self.masks)

        self.factor = torch.ones(self.episode_length, self.n_rollout_threads, self.num_agents, 1, device=self.device)
    ...
    def compute_cost_returns(self, next_cost, value_normalizer=None):
        ...
        for step in reversed(range(self.costs.shape[0])):
            ...
            # gae = delta + self.gamma * self.gae_lambda * self.masks[step + 1] * gae
            # self.cost_returns[step] = gae + value_normalizer.denormalize(self.cost_preds[step])
            cost_t = value_normalizer.denormalize(
                self.cost_preds[step]
            )
            cost_t_next = value_normalizer.denormalize(
                self.cost_preds[step + 1]
            )
            costs_t = self.costs[step]
            ...
            delta = (
                    costs_t
                    + self.gamma * self.masks[step + 1] * cost_t_next
                    - cost_t
            )
            gae = (
                    delta
            ...
            self.cost_returns[step] = gae + cost_t
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None, cost_adv=None):
        ...
        for indices in sampler:
            ...
            if self.factor is None:
                ...
                if self.algo == "mappolag" or self.algo == "matlag":
                    factor_batch = factor[indices]
                    yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch, cost_preds_batch, cost_return_batch, rnn_states_cost_batch, cost_adv_targ, aver_episode_costs
                elif self.algo == "macpo":
                    factor_batch = factor[indices]
    ...
    def feed_forward_generator_transformer(
            self, advantages, num_mini_batch=None, mini_batch_size=None, cost_adv=None
    ):
        """
        ...
        rnn_states_cost = self.rnn_states_cost[:-1].reshape(
            -1, *self.rnn_states_cost.shape[2:]
        )
        ...
        if cost_adv is not None:
            cost_adv = cost_adv.reshape(-1, *cost_adv.shape[2:])
            cost_adv = cost_adv[rows, cols]
        aver_episode_costs = self.aver_episode_costs
        ...
        for indices in sampler:
            ...
            rnn_states_cost_batch = rnn_states_cost[indices].reshape(
                -1, *rnn_states_cost.shape[2:]
            )
            ...
            cost_return_batch = cost_returns[indices].reshape(-1, *returns.shape[2:])
            ...
            if cost_adv is None:
                ...
                cost_adv_targ = cost_adv[indices].reshape(-1, *cost_adv.shape[2:])
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, cost_preds_batch, cost_return_batch, rnn_states_cost_batch, cost_adv_targ, aver_episode_costs
