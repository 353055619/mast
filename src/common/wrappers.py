...
class MultiGoalEnv():
    ...
    def step(
        ...
        rewards, costs, dones, infos = list(rewards.values()), list(costs.values()), list(dones.values()), list(
            infos.values())
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def step_async(self, actions):
        if type(actions) in (list, tuple):  # modify
    ...
    def step_wait(self):
        ...
        obs, share_obs, rews, costs, dones, available_actions = map(
            lambda x: torch.tensor(np.stack(x), device=self.device),
            (obs, share_obs, rews, costs, dones, available_actions)
        )
